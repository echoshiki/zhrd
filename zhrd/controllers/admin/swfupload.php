...
class Swfupload extends adminBase{
...
//客户编辑用户资料
	...
	// echo $_COOKIE['zhrd_session'];
		...
		switch($file_ext){
			...
			case 'gif':
			case 'png':
			case 'bmp': $this->form_imgup( $this->input->post('formid') ); break;
			case 'flv':
			case 'mp4':
			case 'rmvb': $this->form_videoup( $this->input->post('formid') );break;
			default:show_404();
		}
	}
		
    public function form_imgup($form_id)
    {
// $this->db->where('ID',$form_id)->update('FORM', array('THUMB' => ''));exit;
    ...
    public function form_videoup($form_id)
    {
		if($_POST  )
        {
			// $form_id = $this->session->userdata('form_id');
