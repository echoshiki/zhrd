...
class index_model extends CI_Model {
	...
	public $routeUri = array(
						...
						'activitiesListUrl' => '/activities/',
						'activitiesShowUrl' => '/activities/show/',
						'searchUrl'      => '/search',
						'sitemapUrl'     => '/setmap/'
	);
	 ...
	 */	
	public function categoryLeft($pid="48")
	{
		$data = array();
		
		$query = $this->db->select('CATID,CATNAME,TYPE')->where('PARENTID' ,$pid)->get('CATEGORY')->result_array();
	...
	public function getCategoryUrl($query)
	{
		foreach( $query as $k => $v ){
			...
			switch ( $v['TYPE'] ) {
				...
				case 3: $query[$k]['URL'] = $this->routeUri['activitiesListUrl'].$v['CATID']; break;
			}
		}
		return $query;
	}
	...
	public function getShowUrl($query,$type = 1)
	{
		foreach( $query as $k => $v ){
			switch ( $v['TYPEID'] ) {
				...
				case 3: $query[$k]['URL'] = $this->routeUri['activitiesShowUrl'].$v['ID']; break;
			}
		}
		return $query;
	}
